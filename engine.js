// The mode/session state machine — JS build of the TS source (useCoachSession.ts),
// verified by the SAME 16-check test suite (test_engine.mjs). Feed 33 landmarks/frame;
// it returns what the UI shows and logs finished holds/sets. [generated: keep in sync]
//
...
import { handstandScore, isInverted, isHanging, pushupFrame, plankScore, torsoLean,
         RepCounter, SquatCounter, PullupCounter, angleAt, pickSide, chain, EMA } from "./scorer.js";
...
export class CoachEngine {
  session = [];
  onLog;             // wire to the supabase insert
  ...
  log(e) { this.session.push(e); this.onLog?.(e); }
}
